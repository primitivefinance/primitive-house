...
describe("House integration tests using sushi venue", function () {
...
// todo, util function to simply mint options for user on demand
  ...
  beforeEach(async function() {
    ...
    // Deploy uniswap instances
    uniswapFactory = await waffle.deployContract(signers[0], UniswapV2Factory, [signers[0].address], {gasLimit: 9500000})
    ...
    // 5. deploy venue
    venue = await deploy('SushiVenue', { from: signers[0], args: [weth.address, house.address, MultiToken.address, uniswapFactory.address, uniswapRouter.address] })
